...
const {monthlyReportDb,yearlyReportDb}=require("../databaseFuntion/orderQuery");
...
function loadReportPage(req,res){
...
function monthlyReport(req,res){
    ...
    monthlyReportDb(seller,month,year).then(function(data){
...
function yearlyReport(req,res){
    ...
    const year=req.body.year;
    // console.log(year);
   yearlyReportDb(seller,year).then(function(data){
        res.status(200).json(data);
    }).catch(function(err){
        res.status(500).send("ERROR");
    });
