const {addNewProduct,findProductById,findProductsForPage,updateProductDB,deleteProductDB,isThisLastPage, findProductsNotAproved,addProductToApproved,addProductToRejected,findMyProductsDb}=require("../databaseFuntion/productQuery");
...
function getProducts(req, res) {
    ...
    findProductsForPage(page).then(async function (products) {
        const isLast=await isThisLastPage(page+1,'Y');
        if(products==false){
            products=[];
        }
        res.json({products,isLast});
...
function myProducts(req, res) {
    ...
    findMyProductsDb(page,req.session.sid).then(async function (products) {
